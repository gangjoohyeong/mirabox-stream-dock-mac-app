...
| `build` | 빌드 서버 부하 | ssh |
| `image` | 그림 파일을 키에 채운다 | 없음 |
| `text` | 원하는 글자 | 없음 |
| `clock` | 현재 시각 | 없음 |
| `blank` | 아무것도 표시하지 않는다 | 없음 |
...
프로필을 여러 개 두고 각각에 앱을 묶을 수 있다. 그 앱이 앞으로 나오면
자동으로 전환된다. 앱 감시는 NSWorkspace 로 하므로 폴링이 없다.

## 앱으로 만들기

터미널 없이 더블클릭으로 쓰려면 `.app` 번들을 만든다.

```bash
tools/build_app.sh
cp -R "dist/Stream Dock.app" /Applications/
```

PyInstaller 로 묶는다. 결과는 약 113MB 이고 ad-hoc 서명이 붙는다. 개발자
인증서가 없으므로 다른 맥으로 옮기면 Gatekeeper 가 막는다. 그때는 우클릭
후 열기를 한 번 하면 된다. 같은 맥에서 만들어 쓰면 그냥 열린다.

번들은 창 앱이라 stdout 이 버려진다. 로그는 파일로 간다.

```
~/Library/Logs/mirabox/app.log
```

## 로그인 자동 시작

```bash
uv run mirabox-agent install     # launchd 에 등록
uv run mirabox-agent status
uv run mirabox-agent uninstall
```

에이전트는 화면 없는 데몬을 돌린다. 조작 화면을 열면 같은 기기를 두고
다투므로, 화면이 사는 동안에는 에이전트를 잠시 내렸다가 닫을 때 되살린다.

`.app` 으로 돌고 있을 때는 앱 자신을 등록한다. 번들에는 CLI 데몬이 따로
없고 앱이 데몬을 품고 있기 때문이다. 다만 지금은 메뉴바 아이콘이 없어서
창을 닫으면 데몬도 함께 끝난다. 메뉴바 상주는 남은 과제다.
